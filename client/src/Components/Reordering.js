              ...
              <Draggable key={song.id} draggableId={String(song.id)} index={index}>
                {(provided, snapshot) => (
                  <div
                    ...
                    className={`flex items-start justify-between p-4 bg-white border rounded-lg shadow-md hover:shadow-lg cursor-pointer transition ${
                      snapshot.isDragging ? 'bg-blue-100 shadow-lg' : ''
                    }`}
                  >
                    <div className="flex flex-col">
                      <h3 className="text-purple-600 font-semibold text-lg hover:underline">
                        {song.title}
                      </h3>
                      <p className="text-gray-600">
                        Artist: <span className="font-medium">{song.artist}</span>
                      </p>
